  ...
  return (
    ...
    >
      <div className="relative">
        ...
        />
        <div
          className={`absolute h-48 inset-0 flex flex-col items-start justify-end p-5 transition-opacity duration-1000 ${
            isHovered ? "bg-black bg-opacity-60 opacity-100 scale-110" : "opacity-0"
          }`}
        ></div>
      </div>
      <div className="flex flex-col py-5 px-5 gap-4">
        <p className="sm:text-base text-lg font-medium transition-colors text-black duration-1000">
          {title}
        </p>
        {link && ( // Ensure link is not undefined
          <Link
