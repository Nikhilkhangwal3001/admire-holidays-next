...
export default function ResortDetail({ params }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* Header Section */}
      <div className="mb-8">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{resort.title}</h1>
        <div className="flex items-center text-gray-600">
          <span className="mr-4">{resort.city}, {resort.state}, {resort.country}</span>
          <div className="flex items-center">
            <span className="text-yellow-500 mr-1">★</span>
            <span>{resort.average_rating} ({resort.review_count.toLocaleString()} reviews)</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 lg:grid-cols-1 gap-4">
          {resort.public_images.map((img, index) => (
            <Image
              ...
              className={`w-full h-28 object-cover rounded-lg cursor-pointer transition-opacity ${activeImageIndex === index ? 'ring-2 ring-indigo-500' : 'opacity-80 hover:opacity-100'}`}
        ...
        <div className="lg:col-span-2">
          ...
          <section className="mb-8">
            ...
            <div className="bg-gray-50 p-6 rounded-lg">
              ...
              <div className="whitespace-pre-line text-gray-700">{resort.policies}</div>
            </div>
          </section>
        </div>
        ...
        <div>
          <div className="sticky top-4 border border-gray-200 rounded-xl shadow-md p-6">
            <div className="flex justify-between items-center mb-6">
              <span className="text-2xl font-bold text-indigo-600">₹{resort.price_per_night.toLocaleString()}</span>
            ...
            <div className="mt-6 pt-6 border-t border-gray-200">
              <div className="flex justify-between mb-2">
                <span>₹{resort.price_per_night.toLocaleString()} x 1 night</span>
                <span>₹{resort.price_per_night.toLocaleString()}</span>
              </div>
              {resort.discount && (
                <div className="flex justify-between mb-2 text-green-600">
                  <span>Discount ({resort.discount}%)</span>
                  <span>-₹{(resort.price_per_night * resort.discount / 100).toLocaleString()}</span>
                </div>
              )}
              <div className="flex justify-between font-semibold mt-4 pt-4 border-t border-gray-200">
                ...
                <span>
                  ₹{resort.discount 
                    ? (resort.price_per_night - (resort.price_per_night * resort.discount / 100)).toLocaleString() 
